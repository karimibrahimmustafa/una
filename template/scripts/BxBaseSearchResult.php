...
class BxBaseSearchResult extends BxDolSearchResult
{
    ...
    {   
        ...
        if (BxDolProfile::getInstance()) {
        //check for type of profile doctor is 2 patient is 1 or null
            $type = BxDolProfile::getInstance()->getContentType()['type_of_account'];
            if ($type == 2) {
                if ($aData['type_of_account'] != 2) {
                    //don't show the patient
                    return;
                }
            }
        }
        // return http_build_query($aData);
        // return BXDolProfileQuery::getTypeOfAccount();
